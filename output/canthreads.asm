; C:\M68KV6.0 - 800BY480\PROGRAMS\DEBUGMONITORCODE\CANTHREADS.C - Compiled by CC68K  Version 5.00 (c) 1991-2005  Peter J. Fondse
; /*********************************************************************************************
...
; void ThermistorThread(void *);
; void PotentiometerThread(void *);
; void SwitchThread(void *);
; OS_EVENT *DisplayMutex;                          /* Mutex for access to display                   */
; unsigned char Timer2Count;
...
_Timer2_ISR:
       ...
       lea       _printf.L,A2
; // Timer 1: Every 10ms
; if(Timer2Status == 1) {       // Did Timer 1 produce the Interrupt?
       move.b    4194358,D0
       cmp.b     #1,D0
       bne       Timer2_ISR_9
; Timer2Control = 3;        // if so clear interrupt and restart timer
       move.b    #3,4194358
; Timer2Count++ ;           // increment an LED count on PortA with each tick of Timer 1 
...
; Oline1("Micrium uC/OS-II RTOS");
       ...
       jsr       _Oline1
       addq.w    #4,A7
; // InstallExceptionHandler(Timer2_ISR, 30);
; OSInit();
       jsr       _OSInit
...
; OSTaskCreate(ADCThread, OS_NULL, &Task1Stk[STACKSIZE], 11); // highest priority task
       ...
       jsr       (A2)
       add.w     #16,A7
; OSTaskCreate(SwitchThread, OS_NULL, &Task2Stk[STACKSIZE], 12); // highest priority task
       pea       12
       lea       _Task2Stk.L,A0
       add.w     #512,A0
       move.l    A0,-(A7)
       ...
       jsr       (A2)
       add.w     #16,A7
; OSTaskCreate(PotentiometerThread, OS_NULL, &Task3Stk[STACKSIZE], 13);
       pea       13
       lea       _Task3Stk.L,A0
       add.w     #512,A0
       move.l    A0,-(A7)
       clr.l     -(A7)
       pea       _PotentiometerThread.L
       jsr       (A2)
       add.w     #16,A7
; OSTaskCreate(LightThread, OS_NULL, &Task4Stk[STACKSIZE], 14); // lowest priority task
       pea       14
       lea       _Task4Stk.L,A0
       add.w     #512,A0
       move.l    A0,-(A7)
       ...
       jsr       (A2)
       add.w     #16,A7
; OSTaskCreate(ThermistorThread, OS_NULL, &Task5Stk[STACKSIZE], 15); // lowest priority task
       pea       15
       lea       _Task5Stk.L,A0
       add.w     #512,A0
       move.l    A0,-(A7)
       clr.l     -(A7)
       pea       _ThermistorThread.L
       jsr       (A2)
       add.w     #16,A7
; OSStart();
       jsr       _OSStart
...
; while(1) {
ADCThread_1:
; // OSMutexPend(DisplayMutex, 0, &err);  
; ADCRead();
       jsr       _ADCRead
; // OSMutexPost(DisplayMutex);
; OSTimeDly(1);
       pea       1
       jsr       _OSTimeDly
       addq.w    #4,A7
...
; }
; }
; void PotentiometerThread(void *pdata) {
       xdef      _PotentiometerThread
_PotentiometerThread:
       link      A6,#-4
; INT8U err;
; while(1) {  
PotentiometerThread_1:
; OSMutexPend(DisplayMutex, 0, &err); 
       pea       -1(A6)
       clr.l     -(A7)
       move.l    _DisplayMutex.L,-(A7)
...
; OSTimeDly(20);
       ...
       jsr       _OSTimeDly
       addq.w    #4,A7
       bra       PotentiometerThread_1
; }
; }
; void LightThread(void *pdata) {
       xdef      _LightThread
_LightThread:
       link      A6,#-4
; INT8U err;
...
; }
; }
; void ThermistorThread(void *pdata) {
       xdef      _ThermistorThread
_ThermistorThread:
       link      A6,#-4
; INT8U err;
; while(1) {  
...
@canthr~1_8:
       dc.b      10,0
@canthr~1_9:
       dc.b      83,119,105,116,99,104,101,115,32,83,87,91,55
       dc.b      45,48,93,32,61,32,0
@canthr~1_10:
       dc.b      48,0
@canthr~1_11:
       dc.b      49,0
...
@canthr~1_16:
       dc.b      65,68,67,32,84,104,114,101,97,100,32,115,116
       dc.b      97,114,116,101,100,10,0
@canthr~1_17:
       dc.b      65,68,67,32,80,111,116,101,110,116,105,111,109
       dc.b      101,116,101,114,58,32,37,100,10,0
@canthr~1_18:
       dc.b      65,68,67,32,76,105,103,104,116,32,83,101,110
       dc.b      115,111,114,58,32,37,100,10,0
@canthr~1_19:
       dc.b      65,68,67,32,84,104,101,114,109,105,115,116,111
