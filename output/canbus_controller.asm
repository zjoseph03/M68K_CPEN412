...
; // STEP 1: set up a communication link (chip select, interrupts, etc.) between the host controller
...
; Can0_ModeControlReg |= RM_RR_Bit;
       or.b      #1,5242880
; //set interrupt0 to level activated
; Can0_InterruptReg = DISABLE; 
       clr.b     5242886
...
; // enable external interrupts
; // enable all interrupts
; Can1_InterruptReg = ENABLE; 
       move.b    #1,5243398
; Can1_InterruptEnReg = ENABLE; 
       move.b    #1,5243400
       rts
; }
...
; Can0_TxFrameInfo = 0x08;
       move.b    #8,5242912
; Can0_TxBuffer1 = 0xA5;  // id1
       move.b    #165,5242914
; Can0_TxBuffer2 = 0x20;  // id2
       move.b    #32,5242916
; Can0_TxBuffer3 = 0x24;  // data 
       move.b    #36,5242918
; // Can0_TxBuffer10 = 0x58;
; Can0_CommandReg = TR_Bit; // start the transmission
       move.b    #1,5242882
       rts
; }
...
; Can1_TxFrameInfo = 0x08;
       move.b    #8,5243424
; Can1_TxBuffer1 = 0xA5; // ID1
       move.b    #165,5243426
; Can1_TxBuffer2 = 0x20; // ID2
       move.b    #32,5243428
; Can1_TxBuffer3 = 0x74; // DATA1
       move.b    #116,5243430
; // Can1_TxBuffer10 = 0x58;
; Can1_CommandReg = TR_Bit; // start the transmission
       move.b    #1,5243394
       rts
; }
...
; unsigned char recieve_id1; 
; unsigned char recieve_id2; 
...
CanBus0_Receive_3:
; recieve_id1 = Can0_RxBuffer1; 
       move.b    5242914,-3(A6)
; recieve_id2 = Can0_RxBuffer2; 
       move.b    5242916,-2(A6)
; recieved_data = Can0_RxBuffer3; 
       move.b    5242918,-1(A6)
; Can0_CommandReg = RRB_Bit; //stop transmission
       move.b    #4,5242882
; printf("\nCanbus 0 Recieved Data: 0x%X", recieved_data); 
       move.b    -1(A6),D1
       and.l     #255,D1
       move.l    D1,-(A7)
       pea       @canbus~1_1.L
...
CanBus1_Receive_3:
; recieve_id1 = Can1_RxBuffer1; 
       move.b    5243426,-3(A6)
; recieve_id2 = Can1_RxBuffer2; 
       move.b    5243428,-2(A6)
; recieved_data = Can1_RxBuffer3; 
       move.b    5243430,-1(A6)
; Can0_CommandReg = RRB_Bit; //stop transmission
       move.b    #4,5242882
; printf("\nCanbus 1 Recieved Data: 0x%X", recieved_data); 
       move.b    -1(A6),D1
       and.l     #255,D1
       move.l    D1,-(A7)
       pea       @canbus~1_2.L
...
; }
       section   const
@canbus~1_1:
       dc.b      10,67,97,110,98,117,115,32,48,32,82,101,99,105
       dc.b      101,118,101,100,32,68,97,116,97,58,32,48,120
       dc.b      37,88,0
@canbus~1_2:
       dc.b      10,67,97,110,98,117,115,32,49,32,82,101,99,105
       dc.b      101,118,101,100,32,68,97,116,97,58,32,48,120
       dc.b      37,88,0
@canbus~1_3:
...
@canbus~1_4:
       dc.b      13,10,0
@canbus~1_5:
       dc.b      13,10,80,101,114,102,111,114,109,105,110,103
       dc.b      32,67,97,110,98,117,115,32,84,101,115,116,58
       dc.b      32,0
       xref      _printf
