...
; void SPIFlashPollStatusBusy(void) {
       xdef      _SPIFlashPollStatusBusy
_SPIFlashPollStatusBusy:
       link      A6,#-4
; unsigned char status;
; unsigned char dummy;
; do {
SPIFlashPollStatusBusy_1:
; // Start a new read status register command each time
; SPI_CS = 0xFE;
       move.b    #254,4227112
; SPISafeWrite(0x05);  // Send Read Status Register command
...
; void SPIFlashWriteEnable() {
       xdef      _SPIFlashWriteEnable
_SPIFlashWriteEnable:
; SPI_CS = 0xFE;
       move.b    #254,4227112
; SPISafeWrite(0x06);
       pea       6
...
; SPIFlashPollStatusWLE();
       jsr       _SPIFlashPollStatusWLE
       rts
; }
; void SPISendAddress(int c) {
       xdef      _SPISendAddress
_SPISendAddress:
       link      A6,#0
       movem.l   D2/A2,-(A7)
       move.l    8(A6),D2
       lea       _SPISafeWrite.L,A2
...
; if (memoryAddr < 0x10000) {
       cmp.l     #65536,D3
       bhs.s     EEPROMByteWrite_1
; deviceAddr = EEPROM0;
       moveq     #80,D2
       bra.s     EEPROMByteWrite_2
EEPROMByteWrite_1:
; } else {
; deviceAddr = EEPROM1;
       moveq     #81,D2
...
; void EEPROMFlashPageWrite(int* data, unsigned int startingGlobalAddress, unsigned int numBytes, int fullMemory) {
       xdef      _EEPROMFlashPageWrite
_EEPROMFlashPageWrite:
       link      A6,#0
       movem.l   D2/D3/D4/D5/D6/D7/A2/A3/A4/A5,-(A7)
       move.l    12(A6),D4
       lea       _checkTIP.L,A2
...
; if (currentAddress < 0x10000) {
       cmp.l     #65536,D2
       bhs.s     EEPROMFlashPageWrite_1
; blockAddress = startingGlobalAddress; // Lower 64KB block
       move.l    D4,D3
; deviceAddress = EEPROM0;
       moveq     #80,D5
       bra.s     EEPROMFlashPageWrite_2
EEPROMFlashPageWrite_1:
; } else {
; blockAddress = startingGlobalAddress - 0x10000; // Upper 64KB block
       move.l    D4,D0
       sub.l     #65536,D0
       move.l    D0,D3
; deviceAddress = EEPROM1;
       moveq     #81,D5
EEPROMFlashPageWrite_2:
; }
; checkTIP();
       jsr       (A2)
; while (currentAddress < endAddress) {
EEPROMFlashPageWrite_3:
       cmp.l     D6,D2
       bhs       EEPROMFlashPageWrite_5
; // printf("Entered New Page, Current Address: %d, End Address: %d\n", currentAddress, endAddress);
; if (currentAddress == 0x10000) {
       cmp.l     #65536,D2
       bne.s     EEPROMFlashPageWrite_6
; blockAddress = 0; // Reset to beginning of Block 1
       clr.l     D3
       bra.s     EEPROMFlashPageWrite_7
EEPROMFlashPageWrite_6:
; } 
; else {
; blockAddress = (currentAddress < 0x10000) ? currentAddress : (currentAddress - 0x10000);
       cmp.l     #65536,D2
       bhs.s     EEPROMFlashPageWrite_8
       move.l    D2,D0
       bra.s     EEPROMFlashPageWrite_9
EEPROMFlashPageWrite_8:
       move.l    D2,D0
       sub.l     #65536,D0
EEPROMFlashPageWrite_9:
       move.l    D0,D3
EEPROMFlashPageWrite_7:
; }    
; pageFlag = 1;
       moveq     #1,D7
; // START condition
; IIC_TXRX = (deviceAddress << 1) & 0xFE;    // Upper 64KB block
       move.l    D5,D0
       lsl.l     #1,D0
       and.l     #254,D0
       move.b    D0,4227078
...
; checkAck();
       jsr       (A3)
; // Send the two address bytes
; // Send the high-order byte of the address
; IIC_TXRX = (blockAddress >> 8) & 0xFF;
       move.l    D3,D0
       lsr.l     #8,D0
       and.l     #255,D0
       move.b    D0,4227078
...
; checkAck();
       jsr       (A3)
; while (pageFlag) {
EEPROMFlashPageWrite_10:
       tst.l     D7
       beq       EEPROMFlashPageWrite_12
; if (fullMemory) {        
       tst.l     20(A6)
       beq.s     EEPROMFlashPageWrite_13
; IIC_TXRX = data[0];
       move.l    8(A6),A0
       move.l    (A0),D0
       move.b    D0,4227078
       bra.s     EEPROMFlashPageWrite_14
EEPROMFlashPageWrite_13:
; } else {
; IIC_TXRX = data[currIndex];
       ...
       move.l    0(A0,D0.L),D0
       move.b    D0,4227078
EEPROMFlashPageWrite_14:
; }
; if (currentAddress == 0xFFFF) {
       cmp.l     #65535,D2
       bne.s     EEPROMFlashPageWrite_15
; deviceAddress = EEPROM1;  // Switch to next device for next byte
       moveq     #81,D5
EEPROMFlashPageWrite_15:
; }
; if (blockAddress % 128 == 127 || currentAddress == endAddress - 1 || currentAddress == 0xFFFF) {
       ...
       addq.w    #8,A7
       cmp.l     #127,D0
       beq.s     EEPROMFlashPageWrite_19
       move.l    D6,D0
       subq.l    #1,D0
       cmp.l     D0,D2
       beq.s     EEPROMFlashPageWrite_19
       cmp.l     #65535,D2
       bne.s     EEPROMFlashPageWrite_17
EEPROMFlashPageWrite_19:
; IIC_CRSR = STOP | WRITE | IACK;
       move.b    #81,4227080
; pageFlag = 0;
       moveq     #0,D7
...
; wait5ms();
       jsr       _wait5ms
       bra.s     EEPROMFlashPageWrite_18
EEPROMFlashPageWrite_17:
; } else {
; IIC_CRSR = WRITE | IACK;
       move.b    #17,4227080
; checkTIP();
...
; blockAddress++;  // Increment blockAddress WITH currentAddress
       addq.l    #1,D3
       bra       EEPROMFlashPageWrite_10
EEPROMFlashPageWrite_12:
       bra       EEPROMFlashPageWrite_3
EEPROMFlashPageWrite_5:
       movem.l   (A7)+,D2/D3/D4/D5/D6/D7/A2/A3/A4/A5
       unlk      A6
       rts
; }
...
; if (readAddr < 0x10000) {
       cmp.l     #65536,D3
       bhs.s     EEPROMRandomRead_1
; deviceAddr = EEPROM0;
       moveq     #80,D2
       bra.s     EEPROMRandomRead_2
EEPROMRandomRead_1:
; } else {
; deviceAddr = EEPROM1;
       moveq     #81,D2
...
EEPROMRandomRead_3:
       move.b    4227080,D0
       and.b     #1,D0
       bne.s     EEPROMRandomRead_5
       bra       EEPROMRandomRead_3
EEPROMRandomRead_5:
; IIC_CRSR = 0;
       clr.b     4227080
; readData = IIC_TXRX;
...
; checkTIP();
       jsr       (A2)
; IIC_TXRX = ((EEPROM0 << 1) & 0xFE); // Send EEPROM address with write bit
       move.b    #160,4227078
...
; checkAck(); 
       jsr       (A3)
; IIC_TXRX = (currAddr >> 8) & 0xFF; // Send high byte of address
       move.l    D2,D0
       lsr.l     #8,D0
       and.l     #255,D0
       move.b    D0,4227078
; IIC_CRSR = WRITE | IACK; // Write command
       move.b    #17,4227080
; checkTIP();
       jsr       (A2)
; checkAck();
       jsr       (A3)
; IIC_TXRX = (currAddr & 0xFF); // Send low byte of address
       move.l    D2,D0
       and.l     #255,D0
       move.b    D0,4227078
; IIC_CRSR = WRITE | IACK; // Write command
...
; checkAck();
       jsr       (A3)
; // Repeated start condition here:
; IIC_TXRX = ((EEPROM0 << 1) | 0x01); // (A3) Send EEPROM address with read bit
       move.b    #161,4227078
; IIC_CRSR = START | WRITE | IACK; // Start condition with read bit
       move.b    #145,4227080
; checkTIP();
       jsr       (A2)
; checkAck();
...
EEPROMReadBlock0_1:
       cmp.l     D4,D2
       bhs       EEPROMReadBlock0_3
; if (currAddr == 0xFFFF || currAddr == endAddr - 1) {
       cmp.l     #65535,D2
       beq.s     EEPROMReadBlock0_6
       move.l    D4,D0
       subq.l    #1,D0
       cmp.l     D0,D2
       bne.s     EEPROMReadBlock0_4
EEPROMReadBlock0_6:
; IIC_CRSR = STOP | READ | IACK | NACK; // Stop condition with read bit
       move.b    #105,4227080
; exitFlag = 1;
       moveq     #1,D5
       bra.s     EEPROMReadBlock0_5
EEPROMReadBlock0_4:
; } else {
...
EEPROMReadBlock0_7:
       move.b    4227080,D0
       and.b     #1,D0
       bne.s     EEPROMReadBlock0_9
       bra       EEPROMReadBlock0_7
EEPROMReadBlock0_9:
...
; if (exitFlag) {
       tst.l     D5
       beq.s     EEPROMReadBlock0_12
; printf("\r\n Exiting EEPROM Read Block 0\n");
       pea       @m68kde~1_31.L
       jsr       _printf
       addq.w    #4,A7
; break;
       bra.s     EEPROMReadBlock0_3
EEPROMReadBlock0_12:
       addq.l    #1,D2
       bra       EEPROMReadBlock0_1
EEPROMReadBlock0_3:
       movem.l   (A7)+,D2/D3/D4/D5/A2/A3
       unlk      A6
       rts
...
; checkTIP();
       jsr       (A2)
; IIC_TXRX = ((EEPROM1 << 1) & 0xFE); // Send EEPROM address with write bit
       move.b    #162,4227078
; IIC_CRSR = START | WRITE | IACK; // Start condition with write bit
       move.b    #145,4227080
; checkTIP();
...
; checkAck();
       jsr       (A3)
; // Repeated start condition here:
; IIC_TXRX = ((EEPROM1 << 1) | 0x01); // (A3) Send EEPROM address with read bit
       move.b    #163,4227078
; IIC_CRSR = START | WRITE | IACK; // Start condition with read bit
       move.b    #145,4227080
; checkTIP();
...
EEPROMReadBlock1_1:
       cmp.l     -8(A6),D2
       bhs       EEPROMReadBlock1_3
; if (currAddr == 0xFFFF) {
       cmp.l     #65535,D2
       bne.s     EEPROMReadBlock1_4
; IIC_CRSR = STOP | READ | IACK | NACK; // Stop condition with read bit
       move.b    #105,4227080
; exitFlag = 1;
       moveq     #1,D3
       bra.s     EEPROMReadBlock1_5
EEPROMReadBlock1_4:
; } else {
; IIC_CRSR = (READ | IACK) & (~NACK); // Read command
       move.b    #33,4227080
...
EEPROMReadBlock1_6:
       tst.b     4227080
       bne.s     EEPROMReadBlock1_9
       moveq     #1,D0
       bra.s     EEPROMReadBlock1_10
EEPROMReadBlock1_9:
       clr.l     D0
EEPROMReadBlock1_10:
       and.b     #1,D0
       beq.s     EEPROMReadBlock1_8
       bra       EEPROMReadBlock1_6
EEPROMReadBlock1_8:
; IIC_CRSR = 0; // Clear IF flag
       clr.b     4227080
; readData = IIC_TXRX; // Read data from EEPROM
...
; if (exitFlag) {
       tst.l     D3
       beq.s     EEPROMReadBlock1_13
; printf("\r\n Exiting EEPROM Read Block 1\n");
       pea       @m68kde~1_33.L
       jsr       _printf
       addq.w    #4,A7
; break;
       bra.s     EEPROMReadBlock1_3
EEPROMReadBlock1_13:
       addq.l    #1,D2
       bra       EEPROMReadBlock1_1
EEPROMReadBlock1_3:
       movem.l   (A7)+,D2/D3/A2/A3
       unlk      A6
       rts
...
; int EEPROMSequentialRead(unsigned int startGlobalAddr, int readLen) {
       xdef      _EEPROMSequentialRead
_EEPROMSequentialRead:
       link      A6,#0
       movem.l   D2/A2,-(A7)
       lea       12(A6),A2
       move.l    8(A6),D2
...
; if (startGlobalAddr <= 0xFFFF) {
       cmp.l     #65535,D2
       bhi.s     EEPROMSequentialRead_1
; EEPROMReadBlock0(startGlobalAddr, &readLen);
       move.l    A2,-(A7)
       move.l    D2,-(A7)
       jsr       _EEPROMReadBlock0
       addq.w    #8,A7
; if (readLen > 0) {
       move.l    (A2),D0
       cmp.l     #0,D0
       ble.s     EEPROMSequentialRead_3
; EEPROMReadBlock1(0, &readLen);
       move.l    A2,-(A7)
       clr.l     -(A7)
       jsr       _EEPROMReadBlock1
       addq.w    #8,A7
EEPROMSequentialRead_3:
       bra.s     EEPROMSequentialRead_2
EEPROMSequentialRead_1:
; }
; } else {
; EEPROMReadBlock1(startGlobalAddr - 0x10000, &readLen);
       move.l    A2,-(A7)
       move.l    D2,D1
       sub.l     #65536,D1
       move.l    D1,-(A7)
...
_I2CTest:
       ...
       lea       _EEPROMSequentialRead.L,A4
       lea       __getch.L,A5
; unsigned int i, j;
; unsigned int readData;
; char c, text[150];
...
; switch (intBuffer) {
       ...
       bhs.s     I2CTest_7
       asl.l     #1,D0
       move.w    I2CTest_9(PC,D0.L),D0
       jmp       I2CTest_9(PC,D0.W)
I2CTest_9:
       dc.w      I2CTest_10-I2CTest_9
       dc.w      I2CTest_11-I2CTest_9
       dc.w      I2CTest_12-I2CTest_9
       dc.w      I2CTest_13-I2CTest_9
I2CTest_10:
; case 0:
; dataPattern = 0x00; break;
       clr.l     D2
...
; switch (intBuffer) {
       ...
       bhs.s     I2CTest_21
       asl.l     #1,D0
       move.w    I2CTest_23(PC,D0.L),D0
       jmp       I2CTest_23(PC,D0.W)
I2CTest_23:
       dc.w      I2CTest_24-I2CTest_23
       dc.w      I2CTest_25-I2CTest_23
       dc.w      I2CTest_26-I2CTest_23
       dc.w      I2CTest_27-I2CTest_23
I2CTest_24:
; case 0:
; dataPattern = 0x00; break;
       clr.l     D2
...
; *Start = 0x00;  // terminate with a null
       ...
       rts
; }
; // Sourced from this stack overflow post: https://stackoverflow.com/questions/7898215/how-can-i-clear-an-input-buffer-in-c
; void clearInputBuffer() {
       xdef      _clearInputBuffer
_clearInputBuffer:
       link      A6,#-4
; int c;
; while ((c = getchar()) != '\n' && c != EOF);
clearInputBuffer_1:
       ...
       move.l    D0,-4(A6)
       cmp.l     #10,D0
       beq.s     clearInputBuffer_3
       move.l    -4(A6),D0
       cmp.l     #-1,D0
       beq.s     clearInputBuffer_3
       bra       clearInputBuffer_1
clearInputBuffer_3:
       unlk      A6
       rts
; }
...
@m68kde~1_24:
       ...
       dc.b      73,32,70,108,97,115,104,46,46,46,46,0
@m68kde~1_25:
       dc.b      13,10,32,82,101,97,100,32,68,97,116,97,58,32
       dc.b      37,48,56,120,32,10,0
@m68kde~1_26:
       dc.b      73,73,67,95,67,82,83,82,32,84,114,97,110,115
       dc.b      109,105,116,32,40,66,105,110,97,114,121,41,58
       dc.b      32,0
@m68kde~1_27:
       dc.b      37,100,0
@m68kde~1_28:
       dc.b      10,0
@m68kde~1_29:
       dc.b      69,69,80,82,79,77,32,70,108,97,115,104,32,80
       dc.b      97,103,101,32,87,114,105,116,101,58,32,83,116
       dc.b      97,114,116,32,65,100,100,114,101,115,115,58
       dc.b      32,48,120,37,120,44,32,110,117,109,98,121,116
       dc.b      101,115,58,32,37,100,10,0
@m68kde~1_30:
       dc.b      13,10,32,66,108,111,99,107,32,48,32,65,100,100
       dc.b      114,101,115,115,58,32,48,120,37,48,52,88,58
       dc.b      32,37,100,10,0
@m68kde~1_31:
       dc.b      13,10,32,69,120,105,116,105,110,103,32,69,69
       dc.b      80,82,79,77,32,82,101,97,100,32,66,108,111,99
       dc.b      107,32,48,10,0
@m68kde~1_32:
       dc.b      13,10,32,66,108,111,99,107,32,49,32,65,100,100
       dc.b      114,101,115,115,58,32,48,120,37,48,52,88,58
       dc.b      32,37,100,10,0
@m68kde~1_33:
       dc.b      13,10,32,69,120,105,116,105,110,103,32,69,69
       ...
       dc.b      107,32,49,10,0
@m68kde~1_34:
       dc.b      13,10,32,73,50,67,32,84,101,115,116,10,0
@m68kde~1_35:
       dc.b      13,10,73,110,105,116,105,97,108,105,122,105
       dc.b      110,103,32,73,50,67,46,46,46,10,0
@m68kde~1_36:
       dc.b      13,10,67,104,111,111,115,101,32,111,112,116
       dc.b      105,111,110,58,10,0
@m68kde~1_37:
       dc.b      49,32,82,101,97,100,32,83,105,110,103,108,101
       dc.b      32,66,121,116,101,32,102,114,111,109,32,69,69
       dc.b      80,82,79,77,10,0
@m68kde~1_38:
       dc.b      50,32,87,114,105,116,101,32,83,105,110,103,108
       dc.b      101,32,66,121,116,101,32,116,111,32,69,69,80
       dc.b      82,79,77,10,0
@m68kde~1_39:
       dc.b      51,32,82,101,97,100,32,66,108,111,99,107,32
       dc.b      111,102,32,68,97,116,97,32,102,114,111,109,32
       dc.b      69,69,80,82,79,77,10,0
@m68kde~1_40:
       dc.b      52,32,87,114,105,116,101,32,66,108,111,99,107
       dc.b      32,111,102,32,68,97,116,97,32,116,111,32,69
       dc.b      69,80,82,79,77,10,0
@m68kde~1_41:
       dc.b      13,10,82,101,97,100,32,83,105,110,103,108,101
       dc.b      32,66,121,116,101,10,0
@m68kde~1_42:
       dc.b      69,110,116,101,114,32,97,100,100,114,101,115
       dc.b      115,32,40,104,101,120,41,58,32,48,120,0
@m68kde~1_43:
       dc.b      13,10,82,101,97,100,32,102,114,111,109,32,97
       dc.b      100,100,114,101,115,115,32,48,120,37,88,58,32
       dc.b      48,120,37,48,50,88,10,0
@m68kde~1_44:
       dc.b      13,10,87,114,105,116,101,32,83,105,110,103,108
       dc.b      101,32,66,121,116,101,10,0
@m68kde~1_45:
       dc.b      13,10,69,110,116,101,114,32,97,100,100,114,101
       dc.b      115,115,32,40,104,101,120,41,58,32,48,120,0
@m68kde~1_46:
       dc.b      10,69,110,116,101,114,32,119,104,105,99,104
       ...
       dc.b      109,101,109,111,114,121,58,10,0
@m68kde~1_47:
       dc.b      32,48,58,32,48,120,48,48,10,32,49,58,32,48,120
       dc.b      66,50,10,32,50,58,32,48,120,67,51,10,32,51,58
       dc.b      32,48,120,68,52,10,32,52,58,32,67,117,115,116
       dc.b      111,109,32,118,97,108,117,101,10,0
@m68kde~1_48:
       dc.b      83,101,108,101,99,116,32,112,97,116,116,101
       dc.b      114,110,0
@m68kde~1_49:
       dc.b      87,114,111,116,101,32,48,120,37,48,50,88,32
       dc.b      116,111,32,97,100,100,114,101,115,115,32,48
       dc.b      120,37,88,10,0
@m68kde~1_50:
       dc.b      82,101,97,100,32,98,97,99,107,58,32,48,120,37
       dc.b      48,50,88,10,0
@m68kde~1_51:
       dc.b      13,10,82,101,97,100,32,66,108,111,99,107,32
       dc.b      111,102,32,68,97,116,97,10,0
@m68kde~1_52:
       dc.b      69,110,116,101,114,32,115,116,97,114,116,32
       dc.b      97,100,100,114,101,115,115,32,40,104,101,120
       dc.b      41,58,32,48,120,0
@m68kde~1_53:
       dc.b      13,10,69,110,116,101,114,32,110,117,109,98,101
       dc.b      114,32,111,102,32,98,121,116,101,115,32,116
       dc.b      111,32,114,101,97,100,32,40,104,101,120,41,58
       dc.b      32,48,120,0
@m68kde~1_54:
       dc.b      13,10,82,101,97,100,105,110,103,32,48,120,37
       dc.b      88,32,40,37,100,41,32,98,121,116,101,115,32
       dc.b      115,116,97,114,116,105,110,103,32,102,114,111
       dc.b      109,32,97,100,100,114,101,115,115,32,48,120
       dc.b      37,88,46,46,46,10,0
@m68kde~1_55:
       dc.b      13,10,87,114,105,116,101,32,66,108,111,99,107
       dc.b      32,111,102,32,68,97,116,97,10,0
@m68kde~1_56:
       dc.b      13,10,69,110,116,101,114,32,110,117,109,98,101
       dc.b      114,32,111,102,32,98,121,116,101,115,32,116
       dc.b      111,32,119,114,105,116,101,32,40,104,101,120
       dc.b      41,58,32,48,120,0
@m68kde~1_57:
       dc.b      13,10,69,110,116,101,114,32,119,104,105,99,104
       dc.b      32,100,97,116,97,32,112,97,116,116,101,114,110
       dc.b      32,121,111,117,32,119,97,110,116,32,116,111
       dc.b      32,119,114,105,116,101,32,105,110,116,111,32
       dc.b      109,101,109,111,114,121,58,10,0
@m68kde~1_58:
       dc.b      32,48,58,32,48,120,48,48,10,32,49,58,32,48,120
       dc.b      66,50,10,32,50,58,32,48,120,67,51,10,32,51,58
       dc.b      32,48,120,68,52,10,32,52,58,32,73,110,99,114
       dc.b      101,109,101,110,116,105,110,103,32,112,97,116
       dc.b      116,101,114,110,10,0
@m68kde~1_59:
       dc.b      83,101,108,101,99,116,101,100,32,105,110,99
       dc.b      114,101,109,101,110,116,105,110,103,32,112,97
       dc.b      116,116,101,114,110,32,40,97,100,100,114,101
       dc.b      115,115,32,38,32,48,120,70,70,41,10,0
@m68kde~1_60:
       dc.b      83,101,108,101,99,116,101,100,32,112,97,116
       dc.b      116,101,114,110,58,32,48,120,37,48,50,88,10
       dc.b      0
@m68kde~1_61:
       dc.b      13,10,80,114,101,112,97,114,105,110,103,32,116
       dc.b      111,32,119,114,105,116,101,32,48,120,37,88,32
       dc.b      40,37,100,41,32,98,121,116,101,115,32,111,102
       dc.b      32,100,97,116,97,32,115,116,97,114,116,105,110
       dc.b      103,32,97,116,32,97,100,100,114,101,115,115
       dc.b      32,48,120,37,88,46,46,46,10,0
@m68kde~1_62:
       dc.b      13,80,114,105,110,116,105,110,103,32,69,110
       dc.b      116,105,114,101,32,69,69,80,82,79,77,10,0
@m68kde~1_63:
       dc.b      13,10,32,65,68,67,32,82,101,97,100,58,32,37
       dc.b      100,10,0
@m68kde~1_64:
...
@m68kde~1_175:
       ...
       dc.b      41,32,58,0
@m68kde~1_176:
       dc.b      10,69,110,116,101,114,32,119,104,97,116,32,115
...
@m68kde~1_177:
       ...
       dc.b      32,121,111,117,32,119,97,110,116,32,116,111
       dc.b      32,119,114,105,116,101,32,105,110,116,111,32
       dc.b      109,101,109,111,114,121,10,32,48,120,48,48,32
...
@m68kde~1_178:
       ...
       dc.b      32,121,111,117,32,119,97,110,116,32,116,111
       dc.b      32,119,114,105,116,101,32,105,110,116,111,32
       dc.b      109,101,109,111,114,121,10,32,48,120,48,48,48
       dc.b      48,32,61,32,48,10,32,48,120,49,50,51,52,32,61
...
@m68kde~1_179:
       ...
       dc.b      32,119,114,105,116,101,32,105,110,116,111,32
       dc.b      109,101,109,111,114,121,10,32,48,120,48,48,48
       dc.b      48,95,48,48,48,48,32,61,32,48,10,32,48,120,65
...
@m68kde~1_195:
       ...
       dc.b      48,48,48,53,53,32,10,32,85,109,97,105,114,32
       dc.b      77,97,122,104,97,114,58,32,50,48,51,51,51,51
       dc.b      48,56,10,0
       section   bss
       xdef      _i
_i:
